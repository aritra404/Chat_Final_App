package aritra.seal.new_chat.utils
...
object EncryptionUtils {
    ...
    fun encryptAESKeyWithRSA(secretKey: SecretKey?, publicKey: PublicKey?): String? {
        try {
            ...
            val rawAesKeyBytes = secretKey.encoded // This should be 32 bytes for AES-256
            Log.d(LOG_TAG, "encryptAESKeyWithRSA: Raw AES key bytes length: ${rawAesKeyBytes.size} bytes (expected 32)")

            val encryptedKeyBytes = cipher.doFinal(rawAesKeyBytes) // This should be 256 bytes for RSA-2048
            Log.d(LOG_TAG, "encryptAESKeyWithRSA: RSA Encrypted key bytes length: ${encryptedKeyBytes.size} bytes (expected 256)")

            val base64EncodedKey = Base64.encodeToString(encryptedKeyBytes, Base64.DEFAULT)
            Log.d(LOG_TAG, "encryptAESKeyWithRSA: Base64 Encoded key string length: ${base64EncodedKey.length} characters (expected approx 344)")
    ...
    fun decryptAESKeyWithRSA(encryptedKey: String?, privateKey: PrivateKey?): SecretKey? {
        try {
            if (encryptedKey == null || privateKey == null) {
                Log.e(LOG_TAG, "Cannot decrypt with null key or data")
                return null
            }
            // No more complex recovery logic, just try to decrypt.
            return attemptDecryptAESKey(encryptedKey, privateKey)
        } catch (e: Exception) {
            Log.e(LOG_TAG, "Decryption failed, will not delete key. Error: ${e.message}")
            e.printStackTrace()
            // Return null. Let the caller handle the fact that decryption failed.
            return null
        }
    }
